...
/// @param[in, out] ac A Ptr to an IRMitsubishiAC object to use.
/// @param[in] state The desired state to send.
void IRac::myprotocol(IRMyProtocolAc *ac, const stdAc::state_t state) {
  ac->begin();
  ac->fromCommon(state);
  ac->send();
}
...
// #endif  // SEND_MIRAGE
    ...
    case MYPROTOCOL:
    {
      IRMyProtocolAc ac(_pin, _inverted, _modulation);
      myprotocol(&ac, send);
      break;
    }
#endif  // SEND_MYPROTOCOL
