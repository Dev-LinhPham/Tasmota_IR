...
enum myprotocol_ac_remote_model_t {
  KKG9AC1 = 1,  // (1) KKG9A-C1 Remote. (Default)
  KKG21BC1,     // (2) KKG21B-C1 Remote.
};
...
// #endif  // SEND_MIRAGE
#if SEND_MYPROTOCOL
  void sendMyProtocol(const unsigned char data[],
                  const uint16_t nbytes = kMyProtocolStateLength,
                  const uint16_t repeat = kMyProtocolMinRepeat);
