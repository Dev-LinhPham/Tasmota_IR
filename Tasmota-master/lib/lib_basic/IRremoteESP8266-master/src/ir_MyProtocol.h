...
#ifndef IR_MYPROTOCOL_H_
#define IR_MYPROTOCOL_H_
...
union MyProtocol120Protocol{
  uint8_t raw[kMyProtocolStateLength];  ///< The state in code form.
  struct {  // Common
    // Byte 0
  ...
  struct {  // KKG29A-C1 remote
    ...
    uint8_t                :1;
    uint8_t Sleep_Kkg21bc1 :1;  // Sleep mode on or off.
    uint8_t                :2;
...
// Constants
const uint8_t kMyProtocolAcHeat =    0b001;  // 1
const uint8_t kMyProtocolAcCool =    0b010;  // 2
const uint8_t kMyProtocolAcDry =     0b011;  // 3
//const uint8_t kMyProtocolAcRecycle = 0b100;  // 4
const uint8_t kMyProtocolAcFan =     0b101;  // 5

const uint8_t kMyProtocolAcFanAuto = 0b00;  // 0
const uint8_t kMyProtocolAcFanHigh = 0b01;  // 1
const uint8_t kMyProtocolAcFanMed =  0b10;  // 2
const uint8_t kMyProtocolAcFanLow =  0b11;  // 3
// const uint8_t kMyProtocolAcKKG29AC1FanAuto = 0b00;  // 0
// const uint8_t kMyProtocolAcKKG29AC1FanHigh = 0b01;  // 1
// const uint8_t kMyProtocolAcKKG29AC1FanLow =  0b10;  // 2
// const uint8_t kMyProtocolAcKKG29AC1FanMed =  0b11;  // 3

const uint8_t kMyProtocolAcMinTemp = 16;  // 16C
const uint8_t kMyProtocolAcMaxTemp = 32;  // 32C
const uint8_t kMyProtocolAcTempOffset = 0x5C;
const uint8_t kMyProtocolAcSensorTempOffset = 20;
const uint8_t kMyProtocolAcSensorTempMax = 43;  // Celsius
...
class IRMyProtocolAc {
 public:
  explicit IRMyProtocolAc(const uint16_t pin, const bool inverted = false,
                      const bool use_modulation = true);
  void stateReset(void);
#if SEND_MYPROTOCOL
  void send(const uint16_t repeat = kMyProtocolMinRepeat);
  ...
  myprotocol_ac_remote_model_t getModel(const bool useRaw = false) const;
  void setModel(const myprotocol_ac_remote_model_t model);
  static myprotocol_ac_remote_model_t getModel(const uint8_t *state);
 ...
 private:
  ...
  /// @endcond
#endif  // UNIT_TEST
  MyProtocol120Protocol _;
  myprotocol_ac_remote_model_t _model;
  void checksum(void);
};
#endif  // IR_MIRAGE_H_
