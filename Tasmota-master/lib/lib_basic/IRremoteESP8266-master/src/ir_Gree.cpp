...
void IRsend::sendGree(const uint8_t data[], const uint16_t nbytes,
  ...
  for (uint16_t r = 0; r <= repeat; r++) { // các TH khi nhập repeat 
    // Block #1
...
void IRGreeAC::fixup(void) {
  setPower(getPower());  // Redo the power bits as they differ between models // vì có nhiều model nên sẽ phải thay cacs bit nguồn 
  checksum();  // Calculate the checksums // kiểm tra tổng 
}
